...
uint32_t  pwmkck[(maxled*24)+50];
void send(){
	int index=0;
	uint32_t all;
	for(int i=0;i<maxled;i++){
		all= set[i][1]<<16|set[i][2]<<8|set[i][3];
		for (int i=23;i>=0;i--){
			if(all&(1<<i))
				// 70 of total period on
				pwmkck[index]=28.8;
			else
				// 20 of total period on
				pwmkck[index]=14.4;
			index++;
	...
	HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_1,(uint32_t*)pwmkck, index);
	while (!datasentflag);
		datasentflag = 0;
